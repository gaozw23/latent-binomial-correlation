...
phase3_fisher_se_tail_diagnostic <- function(pairwise) {
  required <- c("scenario_id", "n", "rho_proposed", "if_se_fisher",
                "projection_active", "floor_active")
  missing <- setdiff(required, names(pairwise))
  if (length(missing)) stop("Phase 3 results are missing: ", paste(missing, collapse = ", "), call. = FALSE)
  selected <- pairwise$scenario_id %in% c("S2", "S6")
  cells <- split(pairwise[selected, ],
                 interaction(pairwise$scenario_id[selected], pairwise$n[selected], drop = TRUE))
  out <- lapply(cells, function(x) {
    se <- x$if_se_fisher
    finite_se <- is.finite(se)
    z <- atanh(x$rho_proposed[is.finite(x$rho_proposed) & abs(x$rho_proposed) < 1])
    empirical_fisher_sd <- sd(z)
    q <- quantile(se[finite_se], c(0.5, 0.75, 0.9, 0.95, 0.99, 1), type = 7, names = FALSE)
    active <- x$floor_active %in% TRUE & finite_se
    inactive <- !(x$floor_active %in% TRUE) & finite_se
    data.frame(
      scenario_id = x$scenario_id[1], n = x$n[1], replicates = nrow(x),
      finite_se_count = sum(finite_se), nonfinite_se_count = sum(!finite_se),
      median_fisher_se = q[1], q75_fisher_se = q[2], q90_fisher_se = q[3],
      q95_fisher_se = q[4], q99_fisher_se = q[5], max_fisher_se = q[6],
      mean_fisher_se = mean(se[finite_se]),
      projection_active_proportion = mean(x$projection_active),
      floor_active_proportion = mean(x$floor_active),
      floor_active_mean_se = if (any(active)) mean(se[active]) else NA_real_,
      floor_active_median_se = if (any(active)) median(se[active]) else NA_real_,
      floor_inactive_mean_se = if (any(inactive)) mean(se[inactive]) else NA_real_,
      floor_inactive_median_se = if (any(inactive)) median(se[inactive]) else NA_real_,
      empirical_fisher_scale_sd = empirical_fisher_sd,
      robust_median_se_to_empirical_fisher_sd = q[1] / empirical_fisher_sd,
      stringsAsFactors = FALSE
    )
  })
  result <- do.call(rbind, out)
  rownames(result) <- NULL
  result[order(result$scenario_id, result$n), , drop = FALSE]
}
